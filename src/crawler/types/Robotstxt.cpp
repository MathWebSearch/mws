...
Robotstxt::Robotstxt(GURL aStartUrl) {
    ...
    int isInBrackets = 0;
    FILE* stream;
    auto robpg = new char[rob.size()];
    strcpy(robpg, rob.c_str());
    stream = fmemopen(robpg, rob.size(), "r");
    const int bufmax = 1000;
    char buf[bufmax];
    while (fgets(buf, bufmax, stream) != nullptr) {
        string line(buf);
        if (line.compare(0, userAgent.size(), userAgent) == 0) {
            if (isInBrackets)
                isInBrackets = 0;
            else if (line.compare(0, userAgent.size() + thisCrawler.size(),
                                  userAgent + thisCrawler) == 0)
                isInBrackets = 1;
        }
 ...
 */
Robotstxt::~Robotstxt() { not_allowed.clear(); }
...
int Robotstxt::is_allowed_by_robots_txt(GURL gurl) {
    ...
    for (it = not_allowed.begin(); it != not_allowed.end(); ++it) {
        ...
        if (gurl.host() == c.host() && gurl.query() == c.query()) {
            cp = c.path();
            if (cp[cp.size() - 1] == '/') {
                if (gurl.path().substr(0, cp.length() - 1) ==
                    cp.substr(0, cp.length() - 1))
                    return 0;
            } else {
                if (gurl.path().substr(0, cp.length()) == cp) return 0;
            }
        }
    }
    return 1;
