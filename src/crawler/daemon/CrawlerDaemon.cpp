...
const char *answerpage = "<html><body>We sent the request <br/> You can send another Crawling Request by clicking <a href=\"/\">here</a>! </body></html>";
 ...
 */
static int
cd_SendPageThroughConnection (struct MHD_Connection *connection, const char *page)
{
    int ret;
 ...
 */
static int
cd_IteratePostRequest ( void *coninfo_cls, 
                        ...
                        size_t size)
{
    ...
    if (0 == strcmp (key, "dontcrawl"))
    {
        if ((size > 0) && (size <= MAXNAMESIZE))
        {
            ...
            con_info->dontcrawl[ con_info->dontcrawlnr ] = NULL;
...
/**
 * @brief: Methos that finalizes the connection and pushes the request on a queue
 */
static void
cd_RequestCompletedCallback ( void *cls, 
                              ...
                              enum MHD_RequestTerminationCode toe)
{
    ...
    {
        if (con_info->urlstart != NULL)
        {
            qNode.urlstart = con_info->urlstart;
            goodRequest = 1;
        }
        if (con_info->count != NULL && atoi( con_info->count ) )
        ...
        if (con_info->urlstart)
            free (con_info->urlstart);
        if (con_info->count)
            free (con_info->count);
        for(int i=0; i < con_info->dontcrawlnr ; ++i)
 ...
 */
static int
cd_AccessHandlerCallback (void *cls, 
                          struct MHD_Connection *connection,
                          const char *url,
                          const char *method,
                          const char *version,
                          ...
                          void **con_cls)
{
    ...
    if (NULL == *con_cls)
    {
        ...
        con_info->count = NULL;
        con_info->dontcrawlnr = 0;
        con_info->dontcrawl[0] = NULL;

        if (0 == strcmp (method, "POST"))
        {
            con_info->postprocessor =
                    MHD_create_post_processor (connection, POSTBUFFERSIZE,
                                               cd_IteratePostRequest, (void *) con_info);
    ...
    if (0 == strcmp (method, "POST"))
    {
        ...
        else if (con_info->urlstart!= NULL)
        {
 ...
 */
int
CrawlerDaemon::start(int aPort,SharedQueue *sqPtr)
{
    ...
    _daemonHandler = MHD_start_daemon(MHD_USE_SELECT_INTERNALLY,
                                      aPort,
                                      NULL, // TODO:      my_MHD_AcceptPolicyCallback,
                                      NULL,
                                      cd_AccessHandlerCallback, // ...      my_MHD_AccessHandlerCallback,
                                      NULL,
                                      MHD_OPTION_CONNECTION_LIMIT,
                                      20,
                                      MHD_OPTION_NOTIFY_COMPLETED,
                                      cd_RequestCompletedCallback,
                                      (void*)sqPtr,
                                      MHD_OPTION_END);
    if (_daemonHandler == NULL)
    {
