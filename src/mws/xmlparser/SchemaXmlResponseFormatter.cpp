...
int SchemaXmlResponseFormatter::writeData(const GenericAnswer* ans,
    ...
    outPtr = xmlOutputBufferCreateIO(fileXmlOutputWriteCallback, nullptr, &ctxt,
                                     nullptr);
    if (outPtr == nullptr) {
    ...
    ret = xmlTextWriterWriteComment(
        writerPtr, BAD_CAST "Schemata generated by " MWS_BUILD);
    if (ret == -1) {
    ...
    ret =
        xmlTextWriterStartElement(writerPtr, BAD_CAST SCHEMA_ANSWSET_MAIN_NAME);
    if (ret == -1) {
    ...
    ret = xmlTextWriterWriteAttribute(
        ...
        BAD_CAST to_string(schemataSet.total).c_str());
    if (ret == -1) {
        PRINT_WARN("Error at xmlTextWriterWriteAttribute\n");
        goto finally;
    }
    ...
    for (const ExprSchema& schema : schemataSet.schemata) {
        ...
        ret = xmlTextWriterWriteAttribute(
            writerPtr, BAD_CAST "coverage",
            BAD_CAST to_string(schema.coverage).c_str());
        if (ret == -1) {
            PRINT_WARN("Error at xmlTextWriterWriteAttribute\n");
            goto finally;
        }
