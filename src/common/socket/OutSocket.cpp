...
OutSocket::OutSocket() : _outsideLen(sizeof(sockaddr_in)), _isOpen(false) {}

OutSocket::OutSocket(string aHost, int aPort)
    : _outsideLen(sizeof(sockaddr_in)),
      _host(std::move(aHost)),
      _port(aPort),
      _isOpen(false) {}
...
int OutSocket::enable() {
    ...
    memcpy(&this->_outsideAddr.sin_addr.s_addr, server->h_addr,
           server->h_length);
    this->_outsideAddr.sin_port = htons(this->_port);
    // Open _fd
    ret = connect(this->_fd, (sockaddr*)&this->_outsideAddr, this->_outsideLen);
    if (ret == -1) goto fail;
...
SocketInfo OutSocket::getInfo() const {
    ...
    ret = getnameinfo((const sockaddr*)&this->_outsideAddr, this->_outsideLen,
                      hostname, MAX_HOSTNAME_LEN, service, MAX_SERVICE_LEN, 0);
    if (ret != 0) {
        perror("getnameinfo");
        goto fail;
    }
